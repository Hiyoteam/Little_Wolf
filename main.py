...
try:
	...
	# 剧情词组:女巫
	witch=[["让人变性","压缩纸片","让星云降价","让鱼变好吃","自动写代码","让预言家的水晶球出现rickroll"],
	...
	def save():
		...
		with open("roottriplist.txt","w") as file:
			file.write(list_)
			file.close()
	...
	# 保存日志
	def log(text):
		with open("allgame.log","a+") as file:
			file.write(text)
			file.close()
	...
	def chan(nick,state=False):
		...
		if not state and nick in data.get("msg"):
			time.sleep(1)
		...
		if "cmd" in wss:
			...
			if cmd=="chat":
				...
				if f"@{botname}" in text:
						...
						if text=="|root":
							...
							try:
								...
								if order=="add":
									...
									rootlist.pop(rootlist.index(slices[1][0]))
									save()
						...
						def auto():
							...
							def loop():
								while True:
									...
									for _ in list(data.get("core").keys()):
										uwu=data["core"][_]
										...
										elif uwu[1]==3 and not data.get("werewolf").get("skip"):
											if uwu[3]:
												time.sleep(2)
												send(f"【{card[3][0]}】请睁眼")
												...
												for _ in data.get("werewolf").get("core"):
												...
												for _ in range(120):
													...
													for _ in data.get("werewolf").get("core"):
														if _ not in data.get("werewolf").get("wait"):
															time.sleep(1)
															send(f"今晚你的同伴猝死了\n{roll(werewolf[6])}的狼群忙于为TA送葬 没有行动",_)

													for _ in data.get("werewolf").get("wait"):
														chan(_)
									...
									if not data.get("witch").get("temp") or data.get("witch").get("temp")[-1]:
										_uwu="是一个平安夜\n"

									if isinstance(data.get("werewolf").get("kill"),str):
										_uwu=f"[{data['werewolf']['kill']}]被【{card[3][0]}】杀死了\n"

									if data.get("witch").get("temp"):
									...
									for _ in range(240):
										...
										for _ in data.get("day").get("wait"):
											if data.get("core").get(_)[1]!=4:
												chan(_)
												data["sudden"].append(_)
							...
							while True:
								if data.get("uwu") and len(data.get("temp"))==len(data.get("group")):
									...
									try:
										...
										for _ in data.get("temp")[len(data["group"])-1:]:
					...
					if data.get("uwu") and data.get("core"):
						if nick in data.get("core"):
							note="[被杀] 悲"

							if data.get("core").get(nick)[3]:
								note="[存活] 喜"

							elif nick in data.get("sudden"):
								note="[猝死] 恼"
						...
						time.sleep(1)
						send(f"【{other['game']}】未开始或在报数")

				elif text=="1":
					time.sleep(0.5)

					if data.get("uwu") and not data.get("core"):
						if nick in data.get("temp"):
				...
				if "from" in wss:
					...
					if data.get("uwu"):
						...
						if not data.get("core") and nick==data.get("owner"):
							allow=[int(_) for _ in uwu.split(" ") if _ in list("1234")]

							if allow==data.get("group"):
								send("牌组和原先相同 不变更",nick)

							elif len(allow)<len(data.get("temp")):
						...
						elif msg[0]=="msg" and nick in data.get("state").get(True):
							if len(msg)<2 and not msg[1].strip():
								send("请写下你的遗言",nick)

							elif "$" in text and wss.get("trip") not in root:
								send("如果说 遗言不允许LaTeX？",nick)

							else:
								if nick in data.get("msg"):
									send("遗言更新成功",nick)
							...
							if nick in data.get("day").get("wait"):
								if uwu in data.get("state").get(True):
								...
								elif uwu in data.get("state").get(False):
							...
							elif nick==data.get("now")[1] or isinstance(data.get("now")[1],list) and nick in data.get("werewolf").get("wait"):
								if data.get("now")[0]==2:
									if uwu==nick:
										...
										if uwu in data.get("state").get(True):
											if uwu in data.get("prophet"):
												...
												if not data.get("core").get(uwu)[2]:
													note=roll(prophet[2])
									...
									if uwu in data.get("state").get(True):
										...
										elif uwu in data.get("werewolf").get("core"):
									...
									if not data.get("werewolf").get("wait"):
										note="你们意见不相同 因此没有杀人"

										if data.get("werewolf").get("kill") and data.get("werewolf").get("kill").count(uwu)==len(data.get("werewolf").get("kill")):
											chan(uwu)
											...
											if uwu in data.get("werewolf").get("core"):
												note=f"你们杀死了同伴[{uwu}]"
										...
										if uwu in data.get("core") and not data.get("core").get(uwu)[3]:
								...
								elif data.get("now")[0]==4:
									if uwu==nick:
										send(f"不允许【{card[4][0]}】自己喝下药水",nick)

									elif uwu in data.get("sudden"):
										send(f"你没必要{roll(general[7])}猝死的人",nick)

									else:
										if uwu in data.get("witch").get("allow"):
							...
							elif nick in data.get("state").get(True):
								send("现在不是你的场合 快去睡觉——",nick)

							elif nick in data.get("state").get(False):
								send("你已经死了",nick)
